...
const ControlPanel: React.FC<ControlPanelProps> = ({ 
  ...
  const statusMessage = STATUS_MESSAGES[status as keyof typeof STATUS_MESSAGES] || status;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <motion.button
          className={`flex items-center justify-center gap-2 px-4 py-2 rounded-md bg-green-600 text-white font-medium 
            ${(!canStart || isRunning) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-700'}`}
          onClick={onStart}
          disabled={!canStart || isRunning}
          variants={buttonVariants}
          whileHover={!canStart || isRunning ? "disabled" : "hover"}
          whileTap={!canStart || isRunning ? "disabled" : "tap"}
        >
          <Play size={16} />
          Start
        </motion.button>
      ...
      >
        ...
        <p className="text-sm text-white">{!canStart ? 'Place some obstacles to begin' : statusMessage}</p>
      </motion.div>
    </motion.div>
  );
};
